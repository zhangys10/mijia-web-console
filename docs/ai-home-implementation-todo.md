...
- [x] 在主要页面挂载右下角 AI 助手按钮。（2026-09-20：`app/components/ai-assistant/`，固定右下角且避开 toast。）
- [x] 未登录点击时引导现有小米登录。
- [x] 桌面显示侧边面板，移动端显示全屏抽屉。
- [x] 展示当前家庭、剩余额度和建议语句。配额 `mode: "disabled"` 时必须显示“配额已停用/不可用”，不能显示为零剩余（2026-09-19 状态：配额实现推迟，见 M3；面板已实现该渲染分支）。
- [x] 支持发送、加载、取消、重试和新会话。（“停止”仅中断本地请求，服务端可能仍在处理；公共停止路由属于未来契约变更。）
- [x] 场景执行展示明确结果，不做虚假乐观更新。
- [ ] 配额不足显示恢复时间且不自动重试。该状态仅在配额实现恢复（M3）后可触发。（2026-09-20：429 分支与恢复时间展示已实现，实际触发依赖 M3。）
- [x] 提供“清除会话”操作并明确其只清除对话记忆。
- [x] 不提供模型 Key 输入框。
- [x] 处理键盘访问、焦点、屏幕阅读器名称及安全区域。

### UI 测试

- [x] 登录/未登录两种入口状态。（`tests/ai-assistant-ui.test.mjs` 源断言 + SSR 检查；交互级验证见本地预览冒烟。）
- [x] 手机与桌面布局。（CSS 断言覆盖 760px 断言、安全区域与 100dvh 抽屉。）
- [x] 发送中禁止重复提交。（single-flight 守卫断言。）
- [x] 错误、空、配额不足和部分成功状态。（错误码映射、tool/scenes 卡片与 429 分支断言。）
- [x] 对话内容不写入 Local Storage，除非未来 ADR 明确允许。（断言组件不使用 LocalStorage。）

路由说明：四个公开 AI 路由现在同时存在 EdgeOne Edge Function（`edge-functions/api/ai/*`）与 Next 路由（`app/api/ai/*`）两种入口，Next 路由是薄委托，复用 handler 工厂与 `process.env`，不复制边界逻辑。
...
- [ ] 用户达到默认日额度后不再调用 Agent。推迟：依赖配额实现（M3）；`AI_QUOTA_ENABLED=false` 期间不产生该行为。
- [ ] unlimited principal 可继续使用并留下 usage 指标。推迟：依赖配额实现（M3）。
- [ ] 用户 A/B 的会话、配额和家庭隔离。
