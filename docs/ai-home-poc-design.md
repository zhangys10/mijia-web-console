...
- `Idempotency-Key`：必填，建议 UUID；同一 key 在 10 分钟内返回相同业务结果。
...
flowchart TD
    ...
    C --> D["POST /api/ai/command"]
    D --> E{"响应状态"}
    E -- 成功 / 部分成功 --> F["朗读 message"]
    E -- 失败 --> G["朗读安全错误提示"]
```
