...
> 状态（2026-09-20）：Phase 6 页面助手已实现（`app/components/ai-assistant/`，含 Edge Function 与 Next 路由双入口）。设计中的“停止生成”目前是本地请求中断——冻结契约没有公共停止路由，服务端该轮对话可能仍会完成；真正的服务端停止属于未来契约变更。
