...
型号规格中的能力必须先按访问方式分类，页面不得把属性和 Action 当作同一种操作：

| 能力 | MIoT access | 设备页 | 场景页 |
| --- | --- | --- | --- |
| 只读属性 | `read` | 读取并展示，不提供控制 | 不可加入场景 |
| 读写属性 | `read + write` | 读取当前值并允许直接修改 | 仅标准白名单且有安全值编辑器时可加入场景 |
| 仅写属性 | `write` | 可直接写入，但不伪造当前值 | 当前不加入场景 |
| Action | `action` | 用户明确触发后立即执行；`inputs` 只关联同服务的参数属性，不产生额外属性写入 | 不复用设备页执行入口；没有可信场景目录时保持只读 |
| Event | `notify/event` | 仅展示能力 | 作为触发条件能力处理，不作为动作 |

属性访问分类由共享领域函数产生；设备页和场景页分别消费对应投影。场景策略可以依赖通用访问分类，通用设备能力不得反向依赖场景模块。

单路实体开关可能只发布物理设备和 switch 服务，而没有单独的 `.sN` 设备记录。当该通道明确具备继电器能力、不是无线模式、没有其他已解析目标，并且按键名或物理设备名能够明确识别为灯具时，设备模型保留一个“名称推断”的普通灯回路。其开关状态和控制地址直接来自 switch 服务；控制关系证据仍标记为推断，不伪造云端设备身份。

### 9.5 场景属性语义

- 场景详情中的属性名称和枚举描述必须来自目标设备的 MIoT 型号规格，不使用场景动作名称猜测属性语义。
- 枚举属性跨设备批量编辑时，以规范化后的枚举标签作为语义键，再映射为每台目标设备自己的枚举值；不得直接复制来源设备的数值。
- 同一设备未改变的属性保留原始 `siid`、`piid` 和枚举值，保证场景无损回存。
- 目标设备缺少同名枚举、存在多个同名枚举或规格不可用时，不生成写入动作，并在界面明确显示不兼容或未知状态。
- 布尔和数值属性继续使用原值，但必须通过目标规格的格式、范围和步长校验。

### 9.6 场景动作目录与编辑

- 单设备动作目录按设备实例 TCA、官方型号目录、MIoT Spec 的顺序解析；上层来源成功返回空列表时不得与下层来源合并。
- TCA 的 `black_dids` 按真实 DID 生效。客户端只接收脱敏动作模板和 opaque key，不接收 `sa_id`、原始 command 或私有 payload。
- 单设备编辑器显示官方目录或 MIoT Spec 的原始 description，不使用当前硬编码中文标签；其他页面保留现有标签，等待统一 i18n。
- 允许编辑已验证的属性动作及无输入 MIoT action。带必填输入、未知输入或规格不一致的 action 保持只读。
- 写入前服务端重新解析目录并校验 DID、型号、动作模板、`siid`/`piid`/`aiid`、固定值及范围；客户端模板 key 不作为可信证据。
- 单动作默认进入单设备编辑；折叠动作默认进入批量编辑。批量属性仍按 MIoT 语义映射到各目标设备。
