...
test("renders the Xiaomi smart home dashboard", async () => {
  ...
  assert.match(html, /正在检查米家连接与家庭数据/, "the first render should describe loading without assuming a signed-in home");
  assert.doesNotMatch(html, /aria-label="选择家庭"/, "the home selector appears after connection state is known");
  assert.match(html, /AI 助手/);
  assert.match(html, /aria-label="AI 助手"/);
  assert.doesNotMatch(html, /3 台设备正在运行|客厅吸顶灯|米家空调|空气净化器/, "the first render must not flash demo devices before authentication resolves");
