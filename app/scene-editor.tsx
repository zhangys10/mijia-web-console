...
import { isSceneWritableProperty, mapScenePropertySemantics, mapScenePropertySemanticsResult, scenePropertySemantic, scenePropertySemantics, type ScenePropertySemantic, type ScenePropertyMappingFailure } from "../lib/xiaomi-scene-properties";
...
function catalogSpecProperty(property:SceneActionCatalogProperty):SpecProperty{return {key:`${property.siid}.${property.piid}`,name:property.name,label:property.label,siid:property.siid,piid:property.piid,format:property.format,readable:true,writable:true,...(property.unit?{unit:property.unit}:{}),...(property.choices?{choices:property.choices}:{}),...(property.range?{range:property.range}:{})}}
...
export default function SceneEditor({ homeId, homeName, devices, sceneId, onClose, onSaved }: Props) {
  ...
  const propertyCatalog=useMemo(()=>{const catalog=new Map<string,{key:string;serviceName:string;propertyName:string;groupLabel:string;label:string;capability:SpecProperty;sourceDid:string}>();for(const device of candidates){const specification=deviceSpecs[device.did];for(const group of specification?.groups??[]){for(const property of group.properties){if(!isSceneWritableProperty(group.name,property))continue;const key=`${group.name}:${property.name}`;if(!catalog.has(key))catalog.set(key,{key,serviceName:group.name,propertyName:property.name,groupLabel:group.label,label:property.label,capability:property,sourceDid:device.did})}}}return [...catalog.values()].sort((left,right)=>left.groupLabel.localeCompare(right.groupLabel,"zh-CN")||left.label.localeCompare(right.label,"zh-CN"))},[candidates,deviceSpecs]);
    ...
    {loading?<div className="scene-editor-state">正在读取可编辑场景…</div>:!draft?<div className="scene-editor-state error">{error||"场景读取失败"}</div>:<>
      <div className="scene-editor-scroll">
        ...
        {draft.actionsEditable&&composerOpen&&<section className={`scene-action-composer scene-action-page ${selectedProperties.length||selectedTargetKeys.length||singleAction?"active":""}`}><div className="scene-editor-section-title"><div><span>＋</span><div><strong>{workingIndices.length?"修改动作":"添加动作"}</strong><small>{composerMode==="single"?"选择设备官方目录中公开的动作":"先设置属性与状态，再选择所有支持的目标"}</small></div></div></div>
          ...
          {composerMode==="batch"&&<>{specsLoading&&<div className="scene-composer-state">正在汇总当前家庭的可写设备能力…</div>}{specsError&&<div className="scene-composer-state error">{specsError}</div>}
...
function PropertyValueEditor({property,value,onChange,rawLabels=false}:{property?:SpecProperty;value:SceneValue;onChange:(value:SceneValue)=>void;rawLabels?:boolean}){
