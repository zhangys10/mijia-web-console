...
export async function GET(request: NextRequest) {
  try {
    ...
    const catalog = await loadSceneActionCatalog(session, homeId!, home.ownerUid, devices.devices);
    const projected = catalog.map(device => ({ ...device, actions: device.actions.map(action => { const result = { ...action }; delete result.sceneActionId; return result; }) }));
    return NextResponse.json({ ok: true, homeId, devices: projected });
  } catch (error) {
    const message = error instanceof Error ? error.message : "UNKNOWN_ERROR";
    console.error("[xiaomi-scene-action-catalog]", JSON.stringify({ error: message }));
