...
export async function GET(request: NextRequest) {
  try {
    ...
    if (properties) {
      ...
      const response = await xiaomiRequest(await readXiaomiSession(value, process.env.XIAOMI_SESSION_SECRET), "/app/miotspec/prop/get", { params: mappings.map(([siid, piid]) => ({ did, siid, piid })) });
    ...
    const response = await xiaomiRequest(await readXiaomiSession(value, process.env.XIAOMI_SESSION_SECRET), "/app/miotspec/prop/get", { params: [{ did, siid, piid }] });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.action === true) {
      ...
      const response = await xiaomiRequest(await readXiaomiSession(value, process.env.XIAOMI_SESSION_SECRET), "/app/miotspec/action", miotActionPayload(body.did, siid, aiid, Array.isArray(body.params) ? body.params : []));
