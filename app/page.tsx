...
type Device = { id:number;did?:string;name:string;home:string;homeId:string;room:string;kind:string;icon:string;on:boolean;status:string;detail:string;color:string;online?:boolean;parentId?:string|null;urn?:string|null;topology?:DeviceTopology|null };
...
export default function Home(){
  ...
  const viewDevices=useMemo(()=>deviceView==="hardware"?homeDevices.filter(device=>device.topology?.relation!=="mapped"||!homeDevices.some(parent=>parent.did===device.parentId)):homeDevices.filter(device=>device.topology?.controlledBy.length||device.kind!=="switch"&&device.topology?.role!=="secondary-panel"),[homeDevices,deviceView]);
  const rooms=useMemo(()=>["全屋",...Array.from(new Set(viewDevices.map(d=>d.room)))],[viewDevices]);
  ...
  const topologySummary=useMemo(()=>({panels:homeDevices.filter(device=>device.kind==="switch"&&device.topology?.relation!=="mapped").length,targets:homeDevices.filter(device=>Boolean(device.topology?.controlledBy.length)).length,secondary:homeDevices.filter(device=>device.topology?.role==="secondary-panel").length,fanout:homeDevices.reduce((total,device)=>total+(device.topology?.channels??[]).filter(channel=>channel.targets.length>1).length,0)}),[homeDevices]);
  ...
  async function loadDevices(notify=false){
    ...
    const mapped:Device[]=data.devices.map((device:{did:string;name:string;home:string;homeId:string;room:string;model:string;online:boolean;parentId?:string|null;urn?:string|null;topology?:DeviceTopology|null},index:number)=>{const type=Object.keys(icons).find(key=>device.model.toLowerCase().includes(key))||(/[单双三四五六]开|开关|面板|副控/.test(device.name)?"switch":"sensor");return{id:index+100,did:device.did,name:device.name,home:device.home||"我的家",homeId:device.homeId||"default",room:device.room||"未分配",kind:type,icon:icons[type],on:false,status:device.online?"在线":"离线",detail:device.model||"米家设备",color:["orange","blue","green","violet","cyan"][index%5],online:device.online,parentId:device.parentId,urn:device.urn,topology:device.topology}});
  ...
  return <main className="shell">
    ...
    {tab==="首页"||tab==="设备"?<>
      ...
      <div className="topology-toolbar"><div className="device-view-switch"><button className={deviceView==="hardware"?"active":""} onClick={()=>{setDeviceView("hardware");setRoom("全屋")}}>⌘ 开关与硬件</button><button className={deviceView==="controlled"?"active":""} onClick={()=>{setDeviceView("controlled");setRoom("全屋")}}>◉ 实际受控设备</button></div><small>{deviceView==="hardware"?"物理面板 → 按键 → 一个或多个受控设备":"受控设备 → 主控回路 + 所有副控按键"}</small></div>
      {connection.connected&&(topologySummary.targets>0||topologySummary.secondary>0)&&<div className="topology-summary"><span><b>{topologySummary.panels}</b> 台物理开关</span><span><b>{topologySummary.targets}</b> 个受控目标</span><span><b>{topologySummary.secondary}</b> 台副控面板</span>{topologySummary.fanout>0&&<span className="fanout-summary"><b>{topologySummary.fanout}</b> 个一控多路按键</span>}</div>}
      ...
      {groupedRooms.map(group=><section className="room-group" key={`${selectedHome}:${deviceView}:${group.name}`}><div className="room-heading"><strong>{group.name}</strong><span>{group.devices.length} {deviceView==="hardware"?"台实体设备":"个实际目标"}</span></div><section className="devices">{group.devices.map(device=>{
        ...
        const power=deviceSettings(device).find(setting=>setting.key==="power"),role=device.topology?.role;
        return <article key={device.id} className={`device-card ${device.online===false?"offline":""} ${role==="primary"?"primary-device":role==="secondary-panel"?"secondary-panel":""}`} role="button" tabIndex={0} onClick={()=>openMappedDevice(device)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();openMappedDevice(device)}}}>
          <div className="device-top"><span className={device.color}>{device.icon}</span>{power&&role!=="secondary-panel"?<button aria-label={`${device.name}${device.on?"关闭":"开启"}`} disabled={operating===`${device.id}:power`||!device.online} className={`switch ${device.on?"on":""}`} onClick={event=>{event.stopPropagation();void applySetting(device,power,!device.on)}}><i/></button>:<span className="readonly-badge">{role==="secondary-panel"?"副控":"只读"}</span>}</div>
          <div className="device-title-row"><h3>{device.name}</h3>{role&&role!=="independent"&&<TopologyBadge role={role}/>}</div><p>{device.status} · {deviceKindLabel(device.kind)}</p><div className="detail">{device.detail}<b>›</b></div>
          {deviceView==="controlled"&&Boolean(device.topology?.controlledBy.length)&&<ControlSources sources={device.topology!.controlledBy} devices={homeDevices} onOpen={item=>void openDevice(item)}/>}
          {deviceView==="hardware"&&role==="secondary"&&device.topology?.parentName&&<div className="parent-link"><span>主控</span><strong>{device.topology.parentName}</strong><small>{device.topology.channelLabel||"按键未公开"}</small></div>}
          {deviceView==="hardware"&&channels.length>0&&<TopologyTree channels={channels} devices={homeDevices} onOpen={item=>openMappedDevice(item)}/>}
          {deviceView==="hardware"&&role==="secondary-panel"&&!channels.length&&<div className="unresolved-topology">整台设备为副控面板；米家暂未返回各按键绑定目标。</div>}
    ...
    {selectedDevice&&<div className="modal-bg" onMouseDown={()=>{specRequest.current++;setSelectedDevice(null)}}><div className="modal device-modal" onMouseDown={event=>event.stopPropagation()}>
      ...
      {deviceSpec.loading?<div className="spec-loading"><span/>正在解析设备公开规格和按键功能</div>:deviceSpec.groups.length?<div className="spec-groups">{deviceSpec.groups.map(group=>{
        ...
        const wireless=group.properties.some(property=>/wireless|button-mode|switch-mode/.test(property.name)&&Boolean(settingValues[property.key]));
        return <details className={`spec-group ${focusedMapping&&mapped.some(device=>device.id===focusedMapping.id)?"focused-group":""}`} key={group.key} open={group.name==="switch"||/panel|binding|relay|wireless|mutual/.test(group.name)||deviceSpec.groups.length<=3}>
          <summary><strong>{group.label}{wireless&&<span className="channel-role">副控按键</span>}</strong><small>{settings.length?`${settings.length} 项设置`:"状态与事件"}{group.events.length?` · ${group.events.length} 个事件`:""}</small></summary>
          {(mapped.length>0||bindings.length>0)&&<div className="channel-targets"><span>{wireless?"副控关联设备":"该按键实际控制"}</span>{mapped.map(item=><strong key={item.id}>{item.name}<small>{item.room}</small></strong>)}{bindings.map(item=><strong key={`${item.targetId}:${item.channelIndex}:${item.channelSiid}`}>{item.targetName}<small>{item.targetRoom}</small></strong>)}</div>}
...
function TopologyBadge({role}:{role:DeviceTopology["role"]}){if(role==="independent")return null;return <span className={`topology-badge ${role}`}>{role==="primary"?"主控":role==="secondary-panel"?"副控面板":"副控映射"}</span>}
function ControlSources({sources,devices,onOpen}:{sources:DeviceControlSource[];devices?:Device[];onOpen?:(device:Device)=>void}){
  ...
  return <div className="control-sources"><small>控制来源 · {sources.filter(source=>source.sourceRole==="primary").length} 主控 / {sources.filter(source=>source.sourceRole==="secondary").length} 副控</small>{ordered.map(source=>{
    const device=devices?.find(item=>item.did===source.sourceId);
    return <button key={`${source.sourceId}:${source.channelIndex}:${source.channelSiid}`} className="control-source-row" type="button" onClick={event=>{event.stopPropagation();if(device&&onOpen)onOpen(device)}} disabled={!device||!onOpen}><span className={`source-role ${source.sourceRole}`}>{source.sourceRole==="primary"?"主控":"副控"}</span><span className="source-identity"><strong>{source.sourceName}</strong>{source.viaName&&<small>经 {source.viaName}</small>}</span><span className="source-channel"><strong>{source.channelIndex!==null?`按键 ${source.channelIndex===0?1:source.channelIndex}`:source.channelSiid!==null?`服务 ${source.channelSiid}`:source.sourceRoom}</strong>{source.targetCount>1&&<small>同时控制 {source.targetCount} 台</small>}</span></button>
  })}</div>
}
function TopologyTree({channels,devices,onOpen}:{channels:DeviceControlChannel[];devices:Device[];onOpen:(device:Device)=>void}){
  return <div className="topology-tree"><div className="topology-tree-label">{channels.length} 个关联按键 · {channels.reduce((total,channel)=>total+channel.targets.length,0)} 个受控目标</div>{channels.map(channel=><div className={`topology-channel ${channel.role}`} key={channel.key}><div className="channel-heading"><span>{channel.label}</span><small>{channel.targets.length>1?`1 → ${channel.targets.length}`:channel.role==="secondary"?"无线副控":"主控映射"}</small></div>{channel.targets.map(target=>{const device=devices.find(item=>item.did===target.id);return <button className="channel-target-row" key={target.id} onClick={event=>{event.stopPropagation();if(device)onOpen(device)}}><span className="target-branch">↳</span><span className="target-identity"><strong>{target.name}</strong><small>{target.room}</small></span>{target.controllerCount>1&&<span className="target-controller-count">{target.controllerCount} 控</span>}</button>})}</div>)}</div>
}
