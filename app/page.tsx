...
import { classifyDeviceKind, groupControlledDevices, isControlDevice, isIndependentSmartDevice, listSwitchChannelTargets, selectDeviceView, type SwitchChannelTarget } from "../lib/device-views";
...
const demo:Device[]=[
  ...
  {id:9,name:"客厅中控",home:"我的家",homeId:"demo",room:"客厅",kind:"controller",icon:"▤",on:true,status:"在线",detail:"xiaomi.controller.oh4w",color:"blue",online:true},
];
...
export default function Home(){
  ...
  async function loadDevices(notify=false,preserveRoom=false){
    ...
    const icons:Record<string,string>={light:"☀",lamp:"♢",aircondition:"❄",acpartner:"❄",airpurifier:"≈",vacuum:"◎",fan:"≈",lock:"▣",curtain:"▥",humidifier:"◌",plug:"ϟ",switch:"ϟ",controller:"▤",camera:"◉",sensor:"↗"};
  ...
  return <main className="shell">
    ...
    {tab==="首页"||tab==="设备"?<>
      ...
      {groupedRooms.map(group=><section className="room-group" key={`${selectedHome}:${deviceView}:${group.name}`}><div className="room-heading"><strong>{group.name}</strong><span>{group.devices.length} {deviceView==="hardware"?"台实体设备":"个实际目标"}</span></div><section className="devices">{group.devices.map(device=>{
        ...
        const role=device.topology?.role,sources=device.topology?.controlledBy??[],memberCount=device.members?.length??1,targetCount=channels.reduce((total,channel)=>total+channel.targets.length,0),controller=isControlDevice(device),central=device.kind==="controller";
...
function deviceKindLabel(kind:string){return({light:"灯光",lamp:"灯光",aircondition:"空调",acpartner:"空调伴侣",airpurifier:"空气净化器",vacuum:"扫拖机器人",fan:"风扇",lock:"智能门锁",curtain:"窗帘",humidifier:"加湿器",plug:"智能插座",switch:"智能开关",controller:"中控屏",camera:"摄像头",sensor:"传感器"} as Record<string,string>)[kind]||"智能设备"}
