...
const demo:Device[]=[
  ...
  {id:7,name:"客厅三开",home:"我的家",homeId:"demo",room:"客厅",kind:"switch",icon:"ϟ",on:true,status:"在线",detail:"xiaomi.switch.demo3",color:"orange",online:true,hardwareRole:"switch"},
  {id:8,name:"主卧中控",home:"我的家",homeId:"demo",room:"主卧",kind:"switch",icon:"ϟ",on:true,status:"在线",detail:"xiaomi.switch.panel",color:"violet",online:true,hardwareRole:"switch"},
...
export default function Home(){
  ...
  async function runScene(scene:ManualScene){
    ...
    }catch(error){message(`场景下发失败：${friendlyError(error instanceof Error?error.message:"UNKNOWN_ERROR")}`)}
  ...
  async function logout(){await fetch("/api/xiaomi/status",{method:"DELETE"});polling.current=false;specRequest.current++;sceneGeneration.current++;setConnection({loading:false,connected:false});setDevices(demo);setHomes([{id:"demo",name:"我的家"}]);setSelectedHome("demo");setScenesByHome({demo:{loading:false,items:demoScenes}});setSceneOperating("");setSelectedDevice(null);setFocusedMapping(null);setDeviceSpec({loading:false,groups:[]});setQr({loading:false});setAuthOpen(false);message("已断开米家云连接")}
  ...
  return <main className="shell">
    ...
    {tab==="首页"||tab==="设备"?<>
      ...
      <Title title="快捷场景" sub={connection.connected?"当前家庭的真实手动场景":"演示场景 · 连接米家后显示真实数据"} action="管理场景 →" onAction={()=>setTab("场景")}/><SceneStateMessage loading={connection.connected&&sceneState.loading} error={connection.connected?sceneState.error:undefined}/><section className="scenes">{quickScenes.map((scene,index)=><button key={scene.id} disabled={!scene.enabled||Boolean(sceneOperating)} aria-busy={sceneOperating===scene.id} onClick={()=>void runScene(scene)}><span className={["orange","blue","violet","indigo"][index%4]}>{sceneGlyph(scene)}</span><div><strong>{scene.name}</strong><small>{scene.enabled?`${scene.actionCount} 个动作${connection.connected?" · 点击下发":" · 演示数据"}`:"已停用"}</small></div><b>{sceneOperating===scene.id?"…":"›"}</b></button>)}</section>{!sceneState.loading&&!sceneState.error&&quickScenes.length===0&&<SceneStateMessage empty/>}</>}
      <DeviceManagement devices={homeDevices} allDevices={devices} homes={homes} selectedHome={selectedHome} room={room} connected={connection.connected} onSelectHome={homeId=>{setSelectedHome(homeId);setRoom("全屋");if(connection.connected)void loadScenes(homeId)}} onSelectRoom={setRoom} onOpenDevice={(device,mappedDevice)=>void openDevice(device,mappedDevice)}/>
    </>:tab==="场景"?<Panel title="场景中心" text={connection.connected?"当前家庭的真实手动场景；执行成功仅表示指令已下发。":"当前为演示数据，连接米家后显示真实手动场景。"}><SceneStateMessage loading={connection.connected&&sceneState.loading} error={connection.connected?sceneState.error:undefined} empty={!sceneState.loading&&currentScenes.length===0}/><div className="panel-grid">{currentScenes.map(scene=><button key={scene.id} disabled={!scene.enabled||Boolean(sceneOperating)} aria-busy={sceneOperating===scene.id} onClick={()=>void runScene(scene)}>{sceneGlyph(scene)}<strong>{scene.name}</strong><small>{sceneOperating===scene.id?"正在下发…":scene.enabled?`${scene.actionCount} 个动作 · 点击${connection.connected?"下发":"演示"}`:"已停用"}</small></button>)}</div></Panel>:tab==="自动化"?<Panel title="自动化" text="根据时间、环境和设备状态，让家自动响应。"><Rule a="日落后" b="有人回家" c="打开玄关灯"/><Rule a="每日 23:30" b="门锁已上锁" c="执行晚安"/></Panel>:<Panel title="家庭能耗" text="查看设备用电趋势，发现节能空间。"><div className="chart">{[44,62,52,78,68,90,64].map((height,index)=><i key={index} style={{height:`${height}%`}}/>)}</div><div className="labels">{["周一","周二","周三","周四","周五","周六","今天"].map(day=><span key={day}>{day}</span>)}</div></Panel>}</section>
