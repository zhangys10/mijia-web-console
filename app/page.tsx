...
function TopologyBadge({role,connectionType}:{role:DeviceTopology["role"];connectionType?:DeviceTopology["connectionType"]}){if(role==="independent")return null;const label=role==="unknown"||connectionType==="unknown"?"模式待确认":role==="primary"?(connectionType==="mixed"?"有线 / 无线":"有线主控"):role==="secondary-panel"?"无线副控":"受控回路";return <span className={`topology-badge ${role} ${connectionType||""}`}>{label}</span>}
function ControlSources({sources,devices,onOpen}:{sources:DeviceControlSource[];devices?:Device[];onOpen?:(device:Device)=>void}){
  ...
  return <div className="control-sources"><small>控制来源 · {sources.filter(source=>source.connectionType==="wired").length} 有线 / {sources.filter(source=>source.connectionType==="wireless").length} 无线</small>{ordered.map(source=>{
    ...
    return <button key={`${source.sourceId}:${source.channelIndex}:${source.channelSiid}`} className="control-source-row" type="button" onClick={event=>{event.stopPropagation();if(device&&onOpen)onOpen(device)}} disabled={!device||!onOpen}><span className={`source-role ${source.sourceRole} ${source.connectionType}`}>{source.connectionType==="wired"?"有线主控":source.connectionType==="wireless"?"无线副控":"模式待确认"}</span><span className="source-identity"><strong>{source.sourceName}</strong>{source.viaName&&<small>经 {source.viaName}</small>}</span><span className="source-channel"><strong>{source.channelIndex!==null?`按键 ${source.channelIndex===0?1:source.channelIndex}`:source.channelSiid!==null?`服务 ${source.channelSiid}`:source.sourceRoom}</strong>{source.targetCount>1&&<small>同时控制 {source.targetCount} 台</small>}</span></button>
  })}</div>
}
