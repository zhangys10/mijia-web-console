...
export default function Home({ initialTab = "首页" }: { initialTab?: string } = {}){
  ...
  async function loadEnvironment(homeId:string){
    if(!connection.connected||!homeId||homeId==="demo")return;
  ...
  return <main className="shell">
    ...
    {selectedDevice&&<div className="modal-bg" onMouseDown={()=>{specRequest.current++;setSelectedDevice(null)}}><div className="modal device-modal" onMouseDown={event=>event.stopPropagation()}>
      ...
      <div className="settings-heading"><strong>型号实际支持的能力</strong><span>{deviceSpec.loading?"读取中…":`${deviceSpec.groups.length?directCapabilityCount(deviceSpec.groups):deviceSettings(selectedDevice).length} 项可操作`}</span></div>
  ...
  return <>
    <div className="panel-grid environment-grid">
      {snapshot.groups.map(group=><article className={`scene-card environment-card`} key={group.metric}>
        <div className="environment-value">{formatEnvironmentValue(group.latest?.value??group.readings[0]?.value??0)}<small>{group.unit}</small></div>
